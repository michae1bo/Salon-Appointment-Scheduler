...
MAIN_MENU() {
  ...
  SERVICE_ID_TO_BOOK=$($PSQL "SELECT service_id FROM services WHERE service_id=$SERVICE_ID_SELECTED")
  if [[ ! $SERVICE_ID_SELECTED =~ ^[0-9]+$ ||  -z $SERVICE_ID_TO_BOOK ]]
  then
    ...
    read SERVICE_TIME
    ADD_APOINTMENT_RESPONSE=$($PSQL "INSERT INTO appointments(customer_id, service_id, time) VALUES($CUSTOMER_ID, $SERVICE_ID_SELECTED, '$SERVICE_TIME')")
